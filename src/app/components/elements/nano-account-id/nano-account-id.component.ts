...
export class NanoAccountIdComponent implements OnChanges {
	...
	ngOnChanges() {
		this.firstCharacters = this.address?.slice(0, this.openingChars).replace('nano_', '')
		this.lastCharacters = this.address?.slice(-this.closingChars)
		this.setMiddle()
	...
	private setMiddle() {
		if (this.middle === 'off') {
			...
			this.middleCharacters = this.address?.slice(this.openingChars, -this.closingChars)
		}
	}
}
